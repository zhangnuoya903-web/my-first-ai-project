...
`data/customer-schema.json` 和 `data/opportunity-schema.json` 定义客户与项目商机基础。客户记录保存在 `data/customers/`，商机记录保存在 `data/opportunities/`。海外客户、销售阶段、预计金额及下一步行动的治理规则见 [docs/SALES_RULES.md](docs/SALES_RULES.md)。

CRM 当前只建立数据基础，不提供页面、不连接外部 API，也不自动修改项目、重量或报价数据。
